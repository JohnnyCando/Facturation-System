...
export class AutenticateService {
  ...
  getProviders() {
    const url_api = "http://localhost:3001/server/users/getProviders"
    return this.http.get(url_api,{headers:this.headers}).pipe(map(data=>data))
      }
  getClients() {
    const url_api = "http://localhost:3001/server/users/getClients"
    return this.http.get(url_api,{headers:this.headers}).pipe(map(data=>data))
  }
  getEmployee() {
  ...
  registerUser(first_name: string, last_name: string, address: string, email: string, password: string, gender_id: number, typeUser_id: number, phone:string,nif:string) {
  ...
  updateUser(first_name: string, last_name: string, address: string, email: string, password: string, gender_id: number, typeUser_id: number, phone:string,nif:string, id: number) {
